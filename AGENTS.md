...
- YouTube player_client별 쿠키: `android_vr`/`android`/`ios`에는 쿠키 인자를 **넘기지 말 것**(`COOKIE_UNSUPPORTED_CLIENTS`). 넘기면 yt-dlp가 `Skipping client "android_vr" since it does not support cookies`로 시도를 통째로 스킵해 1순위가 무력화되고, 쿠키 있는 배포 환경에서 tv/web 실패만 남아 자막이 전면 실패한다(2026-09-16 장애). 쿠키는 임베드(`tv_embedded`/`web_embedded`)·`tv`·`web`에만
- YouTube 캐스케이드(쿠키 있음): `android_vr` → `tv_embedded` → `web_embedded` → `tv` → `web`. 임베드 두 클라이언트는 **쿠키를 받으면서 PO Token 없이 자막을 주는 유일한 경로**라 데이터센터 IP의 마지막 보루 — 제거 금지. **yt-dlp 2026.08.19+ 필요**(2026.06.09에서는 자막 0건, Dockerfile 핀)
- YouTube 에러 분류는 stderr만 본다 — `stripCommandEcho()`로 `Command failed: yt-dlp ... --cookies ...` 에코를 먼저 제거할 것. 제거 안 하면 우리가 넘긴 `--cookies` 인자 때문에 쿠키를 받는 클라이언트(임베드·tv·web)의 **모든** 실패가 COOKIE_EXPIRED로 둔갑한다(2026-09-16 오진)
...
- YouTube 캐스케이드 실패는 클라이언트마다 `ytLog.warn("yt-dlp 시도 실패", { videoId, client, reason, detail })`로 남길 것 — 조용한 `{kind:"cascade"}` 반환으로 되돌리면 어느 클라이언트가 왜 실패했는지 서버에서 복원할 방법이 없다(2026-09-16 진단이 쿠키 오진으로 흐른 직접 원인). `detail`은 `summarizeYtDlpOutput()`을 거칠 것 — 명령 에코를 버리고 ERROR/WARNING 줄을 우선하지 않으면 긴 yt-dlp 명령줄이 길이 제한을 다 먹어 정작 필요한 줄이 잘린다
...
- YouTube 헬스 프로브(`youtube-probe.ts` `_runProbe`)는 서빙 경로 `getTranscript()`를 **그대로 호출** — 별도 yt-dlp 명령으로 복제 금지. 복제하면 캐스케이드(android_vr→tv_embedded→web_embedded→tv→web)·쿠키 폴백이 빠져 "실제 tool은 정상인데 프로브만 down" 늑대소년 발생(과거 쿠키없는 web `--dump-json`이 데이터센터 IP에서 항상 봇 차단). 서킷 브레이커 결합은 의도적(회복 하트비트)
